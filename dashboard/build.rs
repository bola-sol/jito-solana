...
fn main() {
    ...
    if entries.is_empty() {
        ...
        println!(
            "cargo:warning=agave-dashboard: frontend/dist is empty or missing, so the validator \
             will serve a placeholder page. Run `npm install && npm run build` in \
             dashboard/frontend, then build again."
        );
    }
