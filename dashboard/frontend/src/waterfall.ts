 ...
 *
 * Three rows this dashboard draws on newer validators are absent here, because
 * the counters behind them do not exist in this one: the check queue, and both
 * halves of the nonce dedup. Left in, they would read nought for ever, and a
 * nought in this card means the thing did not happen rather than that nothing
 * counts it. Anything restoring them has to restore the counters first.
 *
